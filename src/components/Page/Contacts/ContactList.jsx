...
export default function ContactList() {
    ...
    const [totalPages, setTotalPages] = useState(1); // Assuming you have a total of 5 pages for demonstration
    const [contacts, setContacts] = useState([]);

    // State to trigger re-fetching of contacts
    ...
    async function handlePageChange(newPage) {
        setPage(newPage);
        setReload(!reload); // Trigger re-fetching of contacts
    }
        ...
        <div>
            ...
            <div className="bg-gray-800 bg-opacity-80 rounded-xl shadow-custom border border-gray-700 p-6 mb-8 animate-fade-in">
                ...
                <div id="searchFormContent" className="mt-4">
                    <form onSubmit={handleSearchContact} className="space-y-6">
            ...
            <div className="mt-10 flex justify-center">
                <nav className="flex items-center space-x-3 bg-gray-800 bg-opacity-80 rounded-xl shadow-custom border border-gray-700 p-3 animate-fade-in">
                    ...
                    {getPages().map((pageNum) => {
                        if(pageNum === page) {
                            return <Link to="#" key={pageNum} onClick={() => handlePageChange(pageNum)}  className="px-4 py-2 bg-gradient text-white rounded-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 font-medium shadow-md">
                                {pageNum}
                            </Link>
                        } else {
                            return <Link to="#" key={pageNum} onClick={() => handlePageChange(pageNum)}  className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200">
                                {pageNum}
                            </Link>
                        }
                    })}
